...
var index = (function (props) {
    ...
    var _d = useState(props.addons ? XRENDER : ANTD), type = _d[0]; _d[1];
    var isXrender = !!props.addons;
    var setFieldsValueX = useCallback(function (values) {
        var formData = addons.getValues();
        if (values) {
            var res_1 = __assign({}, formData);
            Object.keys(values).forEach(function (key) {
                set(res_1, key, values[key]);
            });
            addons.setValues(res_1);
        }
    }, []);
    var setFieldValueAntd = useCallback(function (name, value) {
        var _a;
    ...
    var submitX = useCallback(function () {
        console.warn("当前使用的是 submit 方法，已与antd form方法保持一致无promise返回，你可使用validateFields 方法代替");
        addons.submit();
    }, []);
    var getFieldsErrorX = useCallback(function (nameList) {
    ...
    var validateFieldsX = useCallback(function (nameList) {
    ...
    var setFieldsX = useCallback(function (fields) {
    ...
    var setSchemaByPathMixin = useCallback(function (name, schema) {
    ...
    var setSchemaMixin = useCallback(function (schema) {
        console.warn("当前使用的是 xrender setSchemaByPath 方法，仅支持在xrender中使用，请更换实现方式如在onValuesChange中处理联动逻辑");
        if (isXrender) {
            addons.setSchema(schema);
        }
    }, []);
    var resetFieldsMinxin = useCallback(function (fields) {
        console.warn("当前使用的是 antd Form resetFields 方法，仅支持在antd Form中使用，请更换实现方式");
        if (!isXrender) {
            form.resetFields(fields);
        }
    }, []);
    var getFieldInstanceMinxin = useCallback(function (name) {
